...
if(!isset($_GET['category']) OR empty(trim($_GET['category']))) {
  ...
  "<div class='col-s-12 col-l-12'>Du musst eine Kategorie angeben.</div>".PHP_EOL.
  "</div>".PHP_EOL;
} else {
  /**
  ...
  if(mysqli_num_rows($result) == 1) {
    ...
    if($row['description'] !== NULL) {
      $content.= "<div class='row'>".PHP_EOL.
      "<div class='col-s-12 col-l-12'><span class='fas'>&#xf10d;</span> ".nl2br($row['description'])."</div>".PHP_EOL.
      "</div>".PHP_EOL;
    }
    ...
    while($row = mysqli_fetch_array($result)) {
      ...
      "<div class='col-s-12 col-l-3'><a href='".$row['url']."' target='_blank' rel='noopener'><img class='thumb' src='/".($row['thumb'] == NULL ? "src/nothumb.png" : "thumbs/".$row['thumb'])."' alt='Bild'></a></div>".PHP_EOL.
      "<div class='col-s-12 col-l-9'>".PHP_EOL.
      "<div class='row'>".PHP_EOL.
      "<div class='col-s-12 col-l-12 bigger'><span class='bold highlight'>".$row['title']."</span> <span class='italic'>von ".($row['author'] == NULL ? "unbekannt" : "<a href='https://pr0gramm.com/user/".$row['author']."' target='_blank' rel='noopener'>".$row['author']."</a>")."</span><span class='right'><a href='".$row['url']."' target='_blank' rel='noopener'><span class='fas'>&#xf35d;</span> zur Seite</a></span></div>".PHP_EOL.
      "<div class='col-s-12 col-l-12'><span class='fas'>&#xf10d;</span> ".($row['description'] == NULL ? "<span class='italic'>Keine Beschreibung vorhanden</span>" : nl2br($row['description']))."</div>".PHP_EOL.
      "</div>".PHP_EOL.
      "</div>".PHP_EOL.
      "</div>".PHP_EOL;
    ...
    $content.= "<h1>404 - Not Found</h1>".PHP_EOL;
    $content.= "<div class='row'>".PHP_EOL.
    "<div class='col-s-12 col-l-12'>Die Kategorie <span class='italic'>".htmlentities($category, ENT_QUOTES)."</span> existiert nicht.</div>".PHP_EOL.
    "</div>".PHP_EOL;
  }
}
?>
